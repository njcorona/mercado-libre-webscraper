...
url <- "https://listado.mercadolibre.com.uy/rasuradora-electrica_ItemTypeID_N_NoIndex_True"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.com.uy/rasuradora-electrica_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N_NoIndex_True", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.com.uy/rasuradora-electrica_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N_NoIndex_True", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("ElectricRazors_Prod_Uru_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("ElectricRazors_Sell_Uru_Raw.csv", sep = ""))
...
# write_csv(data_legend, "6_12_ElectricRazors_Legend_Uru.csv")
