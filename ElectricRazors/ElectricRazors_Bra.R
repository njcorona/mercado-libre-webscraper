...
url <- "https://lista.mercadolivre.com.br/barbearia-barbeadores-eletricos/_DisplayType_LF_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://lista.mercadolivre.com.br/barbearia-barbeadores-eletricos/_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://lista.mercadolivre.com.br/barbearia-barbeadores-eletricos/_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("ElectricRazors_Prod_Bra_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("ElectricRazors_Sell_Bra_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("ElectricRazors_Prod_Bra.csv", sep = ""))
write_csv(seller_df, date %>% paste("ElectricRazors_Sell_Bra.csv", sep = ""))
print(Sys.time())
