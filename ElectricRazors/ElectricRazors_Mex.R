...
#' 
#' ### Links for electric razors, Mexico, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
date <- "6_23_"
links <- data.frame()
...
url <- "https://listado.mercadolibre.com.mx/higiene-personal-rasuradoras-electricas/rasuradora-electrica_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.com.mx/higiene-personal-rasuradoras-electricas/rasuradora-electrica_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.com.mx/higiene-personal-rasuradoras-electricas/rasuradora-electrica_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("ElectricRazors_Prod_Mex_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("ElectricRazors_Sell_Mex_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("ElectricRazors_Prod_Mex.csv", sep = ""))
write_csv(seller_df, date %>% paste("ElectricRazors_Sell_Mex.csv", sep = ""))
...
# write_csv(data_legend, "6_16_ElectricRazors_Legend_Mex.csv")
