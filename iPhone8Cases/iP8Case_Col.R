...
url <- "https://listado.mercadolibre.com.co/iphone-8-funda_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.com.co/iphone-8-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.com.co/iphone-8-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("iPh8Case_Prod_Col_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("iPh8Case_Sell_Col_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("iPh8Case_Prod_Col.csv", sep = ""))
write_csv(seller_df, date %>% paste("iPh8Case_Sell_Col.csv", sep = ""))
...
# write_csv(data_legend, "6_16_iPh8Case_Legend_Col.csv")
