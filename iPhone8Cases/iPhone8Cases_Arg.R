...
scrapeNodes <- function(test, search_position, name) {
  ...
  # Argentina has periods to designate thousands.  Default interpretation is mistakenly that they're decimals, crops out trailing zeroes.
...
write_csv(df, date %>% paste("iPhone8Cases_Prod_Arg_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("iPhone8Cases_Sell_Arg_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("iPhone8Cases_Prod_Arg.csv", sep = ""))
write_csv(seller_df, date %>% paste("iPhone8Cases_Sell_Arg.csv", sep = ""))
