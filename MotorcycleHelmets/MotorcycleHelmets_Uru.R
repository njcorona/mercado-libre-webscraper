...
#' 
#' ### Links for motorcycle helmets, Argentina, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://motos.mercadolibre.com.uy/accesorios/cascos/casco-de-motocicleta_ITEM*CONDITION_2230284_NoIndex_True"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://motos.mercadolibre.com.uy/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284_NoIndex_True", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://motos.mercadolibre.com.uy/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284_NoIndex_True", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Uru_Raw.csv", sep = ""))
...
list_of_dfs <- vector("list", 3000)
seller_df <- NULL
file_connection <- file(paste(date, "Uru_output.txt", sep = ""), open = "wt")
