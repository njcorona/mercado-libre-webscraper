...
#' 
#' ### Links for motorcycle helmets, Mexico, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://motos.mercadolibre.com.mx/accesorios/cascos/casco-de-motocicleta_ITEM*CONDITION_2230284"
...
num_products_to_scrape <- if (num_products_to_scrape >= 2001) { 40 } else { floor(num_products_to_scrape / 50) * 50 + 1 } # Sets to correct value.  We assume there will be greater than 50 search results.  If there are greater than 2000sx search results, this is set to 2001.  Otherwise, it's set to the greatest possible number of links we can scrape given the number of search results provided.
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://motos.mercadolibre.com.mx/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://motos.mercadolibre.com.mx/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Mex_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("MotorcycleHelmets_Sell_Mex_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Mex.csv", sep = ""))
write_csv(seller_df, date %>% paste("MotorcycleHelmets_Sell_Mex.csv", sep = ""))
print(Sys.time())
