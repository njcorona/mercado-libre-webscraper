...
#' 
#' ### Links for motorcycle helmets, Colombia, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://motos.mercadolibre.com.co/accesorios/cascos/casco-de-motocicleta_ITEM*CONDITION_2230284"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://motos.mercadolibre.com.co/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://motos.mercadolibre.com.co/accesorios/cascos/casco-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_ITEM*CONDITION_2230284", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Col_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("MotorcycleHelmets_Sell_Col_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Col.csv", sep = ""))
write_csv(seller_df, date %>% paste("MotorcycleHelmets_Sell_Col.csv", sep = ""))
print(Sys.time())
