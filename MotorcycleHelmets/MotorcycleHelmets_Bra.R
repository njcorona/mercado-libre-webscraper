...
#' 
#' ### Links for motorcycle helmets, Brazil, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://motos.mercadolivre.com.br/acessorios/capacetes/capacete-de-motocicleta_DisplayType_LF_ITEM*CONDITION_2230284"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://motos.mercadolivre.com.br/acessorios/capacetes/capacete-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ITEM*CONDITION_2230284", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://motos.mercadolivre.com.br/acessorios/capacetes/capacete-de-motocicleta_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ITEM*CONDITION_2230284", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
scrapeNodes <- function(test, search_position, name) {
  ...
  color <- get_html_text(read_html, "input+ .u-clearfix .vip-variation-choose-selected")
  size <- get_html_text(read_html, ".u-clearfix+ .u-clearfix .vip-variation-choose-selected")
  ...
  if (length(color) == 0) {
    color <- gsub("[\t\n]", "", get_html_text(read_html, "[data-id='COLOR_SECONDARY_COLOR'] strong"))
  }
  
  if (length(size) == 0) {
    size <- gsub("[\t\n]", "", get_html_text(read_html, "[data-id='MOTORCYCLE_HELMET_SIZE'] strong"))
  }
  ...
  color <- turn_to_na(color)
  size <- turn_to_na(size)

  df1 <- tibble(search_position, name, list(char_categories), list(char_values), curr_price, orig_price, num_sold, in_stock, num_installments, amt_installments, arrival_time, shipping, free_return, free_return_info, review_avg, num_reviews, seller_link, product_link = test, five_star, four_star, three_star, two_star, one_star, item_id, color, size)
  return(df1)
}
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Bra_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("MotorcycleHelmets_Prod_Bra.csv", sep = ""))
write_csv(seller_df, date %>% paste("MotorcycleHelmets_Sell_Bra.csv", sep = ""))
print(Sys.time())
