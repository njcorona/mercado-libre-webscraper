...
#' 
#' ### Links for bluetooth speakers, Chile, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://listado.mercadolibre.cl/bluetooth-parlante_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.cl/bluetooth-parlante_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.cl/bluetooth-parlante_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("BluetoothSpeakers_Prod_Chi_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("BluetoothSpeakers_Sell_Chi_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("BluetoothSpeakers_Prod_Chi.csv", sep = ""))
write_csv(seller_df, date %>% paste("BluetoothSpeakers_Sell_Chi.csv", sep = ""))
print(Sys.time())
