...
#' 
#' ### Links for bluetooth speakers, Mexico, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://electronica.mercadolibre.com.mx/audio-portatil/bocina/recargables/bluetooth-bocina_ITEM*CONDITION_2230284"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://electronica.mercadolibre.com.mx/audio-portatil/bocina/recargables/bluetooth-bocina_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ITEM*CONDITION_2230284", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://electronica.mercadolibre.com.mx/audio-portatil/bocina/recargables/bluetooth-bocina_Desde_", (i * 50) + 1, sep = ""), "_DisplayType_LF_ITEM*CONDITION_2230284", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("BluetoothSpeakers_Prod_Mex_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("BluetoothSpeakers_Sell_Mex_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("BluetoothSpeakers_Prod_Mex.csv", sep = ""))
write_csv(seller_df, date %>% paste("BluetoothSpeakers_Sell_Mex.csv", sep = ""))
print(Sys.time())
