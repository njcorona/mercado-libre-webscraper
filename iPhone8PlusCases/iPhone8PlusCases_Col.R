...
#' 
#' ### Links for iPhone8+ cases, Colombia, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://listado.mercadolibre.com.co/iphone8-plus-funda#D[A:iphone8%20plus%20funda]"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.com.co/iphone8-plus-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.com.co/iphone8-plus-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, date %>% paste("iPh8PlCase_Prod_Col_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("iPh8PlCase_Sell_Col_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("iPh8PlCase_Prod_Col.csv", sep = ""))
write_csv(seller_df, date %>% paste("iPh8PlCase_Sell_Col.csv", sep = ""))
print(Sys.time())
