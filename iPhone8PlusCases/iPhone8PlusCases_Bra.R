...
#' 
#' ### Links for iPhone8+ cases, Brazil, including the first 50
#' 
## ----scrape_links--------------------------------------------------------
print(Sys.time())
...
url <- "https://lista.mercadolivre.com.br/iphone8-plus-capinha_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://lista.mercadolivre.com.br/iphone8-plus-capinha_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://lista.mercadolivre.com.br/iphone8-plus-capinha_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
scrapeNodes <- function(test, search_position, name) {
  ...
  # Brazil has periods to designate thousands.  Default interpretation is mistakenly that they're decimals, crops out trailing zeroes.
...
write_csv(df, date %>% paste("iPh8PlCase_Prod_Bra_Raw.csv", sep = ""))
...
write_csv(seller_df, date %>% paste("iPh8PlCase_Sell_Bra_Raw.csv", sep = ""))
...
write_csv(df, date %>% paste("iPh8PlCase_Prod_Bra.csv", sep = ""))
write_csv(seller_df, date %>% paste("iPh8PlCase_Sell_Bra.csv", sep = ""))
