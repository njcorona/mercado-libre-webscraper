...
num_products_to_scrape <- if (num_products_to_scrape >= 1151) { 23 } else { floor(num_products_to_scrape / 50) * 50 + 1 } # Sets to correct value.  We assume there will be greater than 50 search results.  If there are greater than 2000sx search results, this is set to 2001.  Otherwise, it's set to the greatest possible number of links we can scrape given the number of search results provided.
...
write_csv(df, "6_6_iPh8PlCase_Raw_Uru_Prod.csv")
```
...
write_csv(seller_df, "6_6_iPh8PlCase_Raw_Uru_Sell.csv")
```
...
write_csv(df, "6_6_iPh8PlCase_Clean_Uru_Prod.csv")
write_csv(seller_df, "6_6_iPh8PlCase_Clean_Uru_Sell.csv")
...
write_csv(data_legend, "6_6_iPh8PlCase_Legend_Uru.csv")
```
