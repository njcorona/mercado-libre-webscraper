...
url <- "https://listado.mercadolibre.com.ar/iphone8-%C3%BE-funda_ItemTypeID_N"
...
for (i in 1:num_products_to_scrape) {
  print(i)
  url <- paste(paste("https://listado.mercadolibre.com.ar/iphone8-%C3%BE-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")
  
  curr_url <- read_html(url) %>% html_nodes("a.item__info-title") %>% html_attr("href")
  
  curr_item <- read_html(paste(paste("https://listado.mercadolibre.com.ar/iphone8-%C3%BE-funda_Desde_", (i * 50) + 1, sep = ""), "_ItemTypeID_N", sep = "")) %>% html_nodes(".main-title") %>% html_text()
...
write_csv(df, "6_12_iPh8PlCase_Raw_Arg_Prod.csv")
```
...
write_csv(seller_df, "6_12_iPh8PlCase_Raw_Arg_Sell.csv")
```
...
sapply(df, 1, function(x) {
  if (is.na(df$num_reviews[x])) { return(x) }
  if (df$num_reviews[x] != 1 && df$num_reviews[x] != 2) { return(x) }
  if (df$num_reviews[x] == 1) {
    df$one_star[x] <- 0
    df$two_star[x] <- 0
    df$three_star[x] <- 0
    df$four_star[x] <- 0
    df$five_star[x] <- 0
    if (df$review_avg[x] == 1) {
       df$one_star[x] <- 1
    } else if (df$review_avg[x] == 2) {
      df$two_star[x] <- 1
    } else if (df$review_avg[x] == 3) {
      df$three_star[x] <- 1
    } else if (df$review_avg[x] == 4) {
      df$four_star[x] <- 1
    } else if (df$review_avg[x] == 5) {
      df$five_star[x] <- 1
    } else {
      print(paste("odd ", x, sep = "- at index x = "))
    }
    return(x)
  } else if (df$num_reviews[x] == 2) {
    df$one_star[x] <- 0
    df$two_star[x] <- 0
    df$three_star[x] <- 0
    df$four_star[x] <- 0
    df$five_star[x] <- 0
    print(x)
    print( df$five_star[x])
    if (df$review_avg[x] == 1) {
       df$one_star[x] <- 2
    } else if (df$review_avg[x] == 1.5) {
      df$one_star[x] <- 1
      df$two_star[x] <- 1
    } else if (df$review_avg[x] == 2) {
      df$two_star[x] <- 2
    } else if (df$review_avg[x] == 2.5) {
      df$two_star[x] <- 1
      df$three_star[x] <- 1
    } else if (df$review_avg[x] == 3) {
      df$three_star[x] <- 2
    } else if (df$review_avg[x] == 3.5) {
      df$three_star[x] <- 1
      df$four_star[x] <- 1
    } else if (df$review_avg[x] == 4) {
      df$four_star[x] <- 2
    } else if (df$review_avg[x] == 4.5) {
      df$four_star[x] <- 1
      df$five_star[x] <- 1
    } else if (df$review_avg[x] == 5) {
      df$five_star[x] <- 2
    ...
    print("check cleaning:  Error ZZ567")
    return(x)
  }
})

write_csv(df, "6_12_iPh8PlCase_Clean_Arg_Prod.csv")
write_csv(seller_df, "6_12_iPh8PlCase_Clean_Arg_Sell.csv")
...
df$search_position <- sapply(df$search_position, function(x) {
...
write_csv(data_legend, "6_12_iPh8PlCase_Legend_Arg.csv")
```
