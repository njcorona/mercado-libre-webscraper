...
scrapeNodes <- function(test, search_position, name) {
  ...
  num_installments_alt <- get_html_text(read_html, ".highlight-info strong")
  ...
  num_installments_alt <- turn_to_na(num_installments_alt)
  ...
  df1 <- tibble(search_position, name, list(char_categories), list(char_values), curr_price, orig_price, num_sold, in_stock, num_installments_alt, arrival_time, shipping, free_return, free_return_info, review_avg, num_reviews, seller_link, test, five_star, four_star, three_star, two_star, one_star, item_id)
  return(df1)
}
```
...
write_csv(df, "7_iPh8PlCase_Prod_Raw_Arg.csv")
```
...
write_csv(seller_df, "7_iPh8PlCase_Sell_Raw_Arg.csv")
```
...
df2 <- apply(df, 2, function(x) { if (length(which(is.na(x) == FALSE)) < 0.1 * length(x)) { FALSE } else { TRUE } })
df <- df[, which(df2 == TRUE)]


write_csv(df, "7_iPh8PlCase_Prod_Clean_Arg.csv")
write_csv(seller_df, "7_iPh8PlCase_Sell_Clean_Arg.csv")
...
write_csv(data_legend, "7_iPh8PlCase_Legend_Arg.csv")
```
